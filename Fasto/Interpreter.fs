...
let rec typeMatch (tpval: Type * Value) : bool =
    ...
    | (Array t, ArrayVal(vals, tp)) ->
        (t = tp)
        && (List.map (fun value -> typeMatch (t, value)) vals |> List.fold (&&) true)
    | (_, _) -> false

let reportBadType (str: string) (want: string) (v: Value) (pos: Position) =
    let msg =
        "Bad type for "
        + str
        + ": expected "
...
let rec bindParams (fargs: Param list) (aargs: Value list) (fid: String) (pdec: Position) (pcall: Position) : VarTable =
    match (fargs, aargs) with
    | ([], []) -> SymTab.empty ()
    | (Param(faid, fatp) :: fargs, v :: aargs) ->
        let vtab = bindParams fargs aargs fid pdec pcall

        if typeMatch (fatp, v) then
            match SymTab.lookup faid vtab with
            | None -> SymTab.bind faid v vtab
            | Some m ->
                raise (
                    MyError(
                        "Formal argument is already in symbol table!"
                        + " In function: "
                        + fid
                        + " formal argument: "
                        + faid,
            ...
            reportWrongType ("argument " + faid + " of function " + fid) fatp v pcall
    | (_, _) -> raise (MyError("Number of formal and actual params do not match in call to " + fid, pcall))
    ...
    (* TODO: project task 1:
     ...
     (`Divide`,...) expression constructors.
        ...
        e.g., `And (e1, e2, pos)` should not evaluate `e2` if `e1` already
              evaluates to false.
  *)
    | Times(e1: Exp<unit>, e2: Exp<unit>, pos: Position) ->
        let res1 = evalExp (e1, vtab, ftab)
        let res2 = evalExp (e2, vtab, ftab)

        match (res1, res2) with
        | (IntVal n1, IntVal n2) -> IntVal(n1 * n2)
        | (IntVal _, _) -> reportWrongType "right operand of *" Int res2 pos
        | (_, _) -> reportWrongType "left operand of *" Int res1 pos
    | Divide(e1: Exp<unit>, e2: Exp<unit>, pos) ->
        let res1 = evalExp (e1, vtab, ftab)
        let res2 = evalExp (e2, vtab, ftab)
        ...
        | (IntVal _, _) -> reportWrongType "right operand of /" Int res2 pos
        | (_, _) -> reportWrongType "left operand of /" Int res1 pos
    | And(e1: Exp<unit>, e2: Exp<unit>, pos: Position) ->
        let res1: Value = evalExp (e1, vtab, ftab)

        match res1 with
        | BoolVal false -> BoolVal false
        | BoolVal true ->
            let res2: Value = evalExp (e2, vtab, ftab)

            match res2 with
            | BoolVal(b: bool) -> BoolVal b
            | _ -> reportWrongType "right operand of &&" Bool res2 pos
        | _ -> reportWrongType "left operand of &&" Bool res1 pos
    | Or(e1: Exp<unit>, e2: Exp<unit>, pos: Position) ->
        let res1: Value = evalExp (e1, vtab, ftab)
        ...
        | BoolVal false ->
            ...
            | _ -> reportWrongType "right operand of ||" Bool res2 pos
        | _ -> reportWrongType "left operand of ||" Bool res1 pos
    | Not(exp: Exp<unit>, pos: Position) ->
        match evalExp (exp, vtab, ftab) with
        | BoolVal b -> BoolVal(not b)
        | _ -> reportWrongType "operand of not" Bool (evalExp (exp, vtab, ftab)) pos
    | Negate(exp: Exp<unit>, pos: Position) ->
        match evalExp (exp, vtab, ftab) with
        | IntVal n -> IntVal(-n)
        | _ -> reportWrongType "operand of negate" Int (evalExp (exp, vtab, ftab)) pos
    | Equal(e1, e2, pos) ->
    ...
    | Index(id, e, tp, pos) ->
        ...
        | (Some(ArrayVal(lst, tp)), IntVal ind) ->
            ...
            if 0 <= ind && ind < len then
                lst.Item ind
            else
                let msg = sprintf "Array index out of bounds! Array length: %i, index: %i" len ind
                raise (MyError(msg, pos))
    ...
    | Iota(e, pos) ->
        ...
        | IntVal size ->
            if size >= 0 then
                ArrayVal(List.map (fun x -> IntVal x) [ 0 .. size - 1 ], Int)
            else
                let msg = sprintf "Argument of \"iota\" is negative: %i" size
                raise (MyError(msg, pos))
    ...
    | Write(exp, t, p) ->
        ...
        | ArrayVal(a, Char) ->
            let mapfun =
                ...
                | CharVal c -> c
                | otherwise ->
                    raise (MyError("Write argument " + ppVal 0 v + " should have been evaluated to string", p))
...
and rtpFunArg (funarg: UntypedFunArg) (ftab: FunTable) (callpos: Position) : Type =
    match funarg with
    | FunName fid ->
        match SymTab.lookup fid ftab with
 ...
 *)
and evalFunArg (funarg: UntypedFunArg, vtab: VarTable, ftab: FunTable, callpos: Position, aargs: Value list) : Value =
    match funarg with
    | (FunName fid) ->
    ...
    | Lambda(rettype, parms, body, fpos) ->
        callFunWithVtable (FunDec("<anonymous>", rettype, parms, body, fpos), aargs, vtab, ftab, callpos)
...
and callFunWithVtable
    ...
    | _ ->
        ...
        if typeMatch (rtp, res) then
            res
        else
            reportWrongType ("result of function \"" + fid + "\"") rtp res pcall
