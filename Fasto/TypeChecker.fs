...
let initFunctionTable: FunTable =
    SymTab.fromList [ ("chr", (Char, [ Int ], (0, 0))); ("ord", (Int, [ Char ], (0, 0))) ]

(* Pretty-printer for function types, for error messages *)
let showFunType (args: Type list, res: Type) : string =
    match args with
    | [] -> " () -> " + ppType res
    | args -> (String.concat " * " (List.map ppType args)) + " -> " + ppType res
...
and checkExp (ftab: FunTable) (vtab: VarTable) (exp: UntypedExp) : (Type * TypedExp) =
    match exp with
    ...
    | ArrayLit(exp :: exps, _, pos) ->
        ...
        let exps_dec =
            List.map
                (fun ei ->
                    let (ti, ei') = checkExp ftab vtab ei

                    if ti <> type_exp then
                        reportTypesDifferent "components of array literal" type_exp ti pos
    ...
    *)
    ...
    | If(pred, e1, e2, pos) ->
        ...
        let target_type =
            if t1 = t2 then
                ...
                reportTypesDifferent "branches of conditional" t1 t2 pos
        ...
        let args_dec =
            List.mapi2
                (fun i ti ai ->
                    ...
                    if ti' <> ti then
                        reportTypeWrong ("function argument #" + string (i + 1)) ti ti' pos
    ...
    | Write(e, _, pos) ->
        ...
        if printable t then
            ...
            reportTypeWrongKind "argument of write" "printable" t pos
    ...
    | Reduce(f, e_exp, arr_exp, _, pos) ->
        ...
        if elem_type <> f_argres_type then
            reportTypesDifferent "operation and array-element types in reduce" f_argres_type elem_type pos
...
and checkFunArg
    ...
    | Lambda(rettype, parms, body, funpos) ->
        ...
        let (FunDec(_, _, _, body', _)) = checkFunWithVtable ftab vtab pos lambda
        (Lambda(rettype, parms, body', pos), rettype, List.map (fun (Param(_, ty)) -> ty) parms)


(* Check a function declaration, but using a given vtable rather
than an empty one. *)
and checkFunWithVtable (ftab: FunTable) (vtab: VarTable) (pos: Position) (fdec: UntypedFunDec) : TypedFunDec =
    ...
    let addParam ptable (Param(pname, ty)) =
        ...
        | Some _ -> reportOther ("Multiple parameters named " + pname) funpos
    ...
    if body_type = rettype then
        (FunDec(fname, rettype, parms, body', pos))
    else
        reportTypeWrong "function body" rettype body_type funpos
...
let updateFunctionTable (ftab: FunTable) (fundec: UntypedFunDec) : FunTable =
 ...
 *)
let checkFun (ftab: FunTable) (fundec: UntypedFunDec) : TypedFunDec =
...
let checkProg (funDecs: UntypedFunDec list) : TypedFunDec list =
    ...
    | Some(ret_type, args, mainpos) -> reportArityWrong "declaration of main" 0 (args, ret_type) mainpos
