...
(iv) function's body,
(v) Position. *)
type FunDec<'T> = FunDec of string * Type * Param list * Exp<'T> * Position
...
let rec ppExp d =
    ...
    | If(e1, e2, e3, _) ->
        ("if ("
         ...
         + "\n"
         + indent d)
    | Apply(id, args, _) -> (id + "(" + (String.concat ", " (List.map (ppExp d) args)) + ")")
    ...
    | Reduce(f, el, lst, _, _) -> "reduce(" + ppFunArg d f + ", " + ppExp d el + ", " + ppExp d lst + ")"
    | Scan(f, acc, arr, _, pos) -> ("scan(" + ppFunArg d f + ", " + ppExp d acc + ", " + ppExp d arr + ")")
...
and ppFunArg d =
    function
    | FunName s -> s
    | Lambda(rtp, args, body, _) -> ("fn " + ppType rtp + " (" + ppParams args + ") => " + ppExp (d + 2) body)

(* pretty prints a function declaration *)
let ppFun d =
    function
    | FunDec(id, rtp, args, body, _) ->
        ("fun "
         + ppType rtp
         + " "
         + id
